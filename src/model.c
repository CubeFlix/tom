...
int layer_init(struct layer *obj, int n_samples, struct matrix *inputs, 
    ...
    {
        // Initialize the quadratic layer.
        struct layer_quadratic* quadratic = calloc(1, sizeof(struct layer_quadratic));
        if (!layer_quadratic_init(quadratic, obj->input_size, obj->output_size, inputs, current_output, current_gradient, d_prev)) {
            free(quadratic);
            return 0;
        }
        obj->trainable = true;
...
int layer_init_optimizer(struct layer* obj, enum optimizer_type type, 
    ...
    case LAYER_QUADRATIC:
        ...
        case OPTIMIZER_SGD:
        {
            ...
            dec = va_arg(ap, double);
	    	nesterov = (bool)va_arg(ap, int);
            struct optimizer_sgd_quadratic* sgd = calloc(1, sizeof(struct optimizer_sgd_quadratic));
            obj->opt.obj = sgd;
            if (!optimizer_sgd_quadratic_init(sgd, obj->obj, lr, mom, dec, nesterov)) {
                free(sgd);
                return 0;
            }
            break;
        ...
        case OPTIMIZER_ADAM:
        {
            ...
            dec = va_arg(ap, double);
            eps = va_arg(ap, double);
            struct optimizer_adam_quadratic* adam = calloc(1, sizeof(struct optimizer_adam_quadratic));
            obj->opt.obj = adam;
            if (!optimizer_adam_quadratic_init(adam, obj->obj, lr, b1, b2, dec, eps)) {
                free(adam);
                return 0;
            }
            break;
		...
		case OPTIMIZER_RMSPROP:
		{
	    	...
	    	eps = va_arg(ap, double);
	    	rho = va_arg(ap, double);
	    	struct optimizer_rmsprop_quadratic* rmsprop = calloc(1, sizeof(struct optimizer_rmsprop_quadratic));
	    	obj->opt.obj = rmsprop;
	    	if (!optimizer_rmsprop_quadratic_init(rmsprop, obj->obj, lr, dec, eps, rho)) {
				free(rmsprop);
				return 0;
	    	}
	    	break;
...
int layer_forward(struct layer *obj, bool training) {
	...
	case LAYER_TANH:
		...
		break;
    case LAYER_QUADRATIC:
        layer_quadratic_forward(obj->obj);
        break;
    default:
        LAST_ERROR = "Invalid layer type.";
        return 0;
...
int layer_backward(struct layer *obj) {
	...
	case LAYER_TANH:
		...
		break;
    case LAYER_QUADRATIC:
        layer_quadratic_backward(obj->obj);
        break;
    default:
        LAST_ERROR = "Invalid layer type.";
        return 0;
...
int layer_free(struct layer *obj) {
    switch (obj->type) {
        ...
        case LAYER_SOFTMAX:
            ...
            break;
        case LAYER_QUADRATIC:
            layer_quadratic_free((struct layer_quadratic*)(obj->obj));
            break;
        case LAYER_SIGMOID:
        case LAYER_RELU:
		case LAYER_LEAKY_RELU:
    ...
    if (obj->opt.obj != NULL) {
        ...
        case LAYER_QUADRATIC:
            switch (obj->opt.type) {
            case OPTIMIZER_SGD:
                optimizer_sgd_quadratic_free((struct optimizer_sgd_quadratic*)(obj->opt.obj));
                break;
            case OPTIMIZER_ADAM:
                optimizer_adam_quadratic_free((struct optimizer_adam_quadratic*)(obj->opt.obj));
                break;
	    	case OPTIMIZER_RMSPROP:
				optimizer_rmsprop_quadratic_free((struct optimizer_rmsprop_quadratic*)(obj->opt.obj));
				break;
            default:
                LAST_ERROR = "Invalid optimizer type.";
                return 0;
...
int layer_update(struct layer* obj) {
    if (obj->opt.obj != NULL) {
        ...
        case LAYER_QUADRATIC:
            switch (obj->opt.type) {
            case OPTIMIZER_SGD:
                optimizer_sgd_quadratic_update(obj->opt.obj, obj->opt.iter);
                break;
            case OPTIMIZER_ADAM:
                optimizer_adam_quadratic_update(obj->opt.obj, obj->opt.iter);
                break;
	    	case OPTIMIZER_RMSPROP:
				optimizer_rmsprop_quadratic_update(obj->opt.obj, obj->opt.iter);
				break;
            default:
                LAST_ERROR = "Invalid optimizer type.";
                return 0;
