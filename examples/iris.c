...
int train(void) {
    ...
    // Save the model.
    FILE *f = fopen("iris_model.dat", "wb");
    QUIT_ON_ERROR(serialize_model(m, f));
    fclose(f);
...
int load(void) {
        struct model m;
        FILE *f = fopen("iris_model.dat", "rb");
        model_init(&m, 150);
        QUIT_ON_ERROR(deserialize_model(&m, f));
        fclose(f);
        //printf("loaded model, version %s\n", tom_version());
        printf("loaded model\n");
