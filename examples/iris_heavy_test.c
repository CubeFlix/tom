...
int main(void) {
    ...
    struct layer* l1 = model_add_layer(m, LAYER_QUADRATIC, input_size, h1_size);
    struct layer* a1 = model_add_layer(m, LAYER_RELU, h1_size, h1_size);
    struct layer* l2 = model_add_layer(m, LAYER_QUADRATIC, h1_size, h2_size);
    ...
    layer_quadratic_init_values(l1->obj, WI_GLOROT_NORMAL, BI_ZEROS, WI_GLOROT_NORMAL);
    layer_quadratic_init_values(l2->obj, WI_GLOROT_NORMAL, BI_ZEROS, WI_GLOROT_NORMAL);
    QUIT_ON_ERROR(model_init_optimizers(m, OPTIMIZER_ADAM, 0.001, 0.9, 0.999, 0.0, 1e-7));

    // Train the model.
    QUIT_ON_ERROR(model_train(m, &X, &Y, 50, true));
    double loss = model_calc_loss(m, &X, &Y);
    printf("loss: %f\n", loss);

    // Model output values.
    struct matrix yhat;
    QUIT_ON_ERROR(matrix_init(&yhat, data_size, h2_size));
    QUIT_ON_ERROR(model_predict(m, &X, &yhat));
    for (int i = 0; i < data_size; i++) {
        printf("y: %f %f %f y-hat: %f %f %f\n", Y.buffer[i * h2_size], Y.buffer[i * h2_size + 1], Y.buffer[i * h2_size + 2], \
            yhat.buffer[i * h2_size], yhat.buffer[i * h2_size + 1], yhat.buffer[i * h2_size + 2]);
    }
